...
export default function PurchGroupChart({ data, isLoading }) {
  ...
  return (
    <ResponsiveContainer width="100%" height={220}>
      <BarChart data={data} margin={{ left: 0, right: 10, top: 4, bottom: 4 }}>
        ...
        <XAxis dataKey="Purchasing Group" tick={{ fill: '#94a3b8', fontSize: 11 }} axisLine={false} tickLine={false} />
        <YAxis tickFormatter={v => formatINR(v)} tick={{ fill: '#64748b', fontSize: 11 }} axisLine={false} tickLine={false} width={70} />
        <Tooltip content={<CustomTooltip formatter={v => formatINR(v)} />} cursor={{ fill: 'rgba(139,92,246,0.08)' }} />
        <Bar dataKey="Total_Spend_INR" name="Spend INR" radius={[6, 6, 0, 0]} maxBarSize={52}>
          {data.map((_, i) => <Cell key={i} fill={CHART_COLORS[(i + 1) % CHART_COLORS.length]} fillOpacity={0.85} />)}
        </Bar>
      </BarChart>
    </ResponsiveContainer>
  )
